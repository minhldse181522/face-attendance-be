...
@CommandHandler(BsCreateUserBranchCommand)
export class CreateUserBranchService
  implements ICommandHandler<BsCreateUserBranchCommand>
{
  constructor(
  ...
  async execute(
    command: BsCreateUserBranchCommand,
  ): Promise<CreateUserBranchServiceResult> {
    const { userContractCode, branchCode } =
      command.getExtendedProps<BsCreateUserBranchCommand>();
    ...
    const commandProps = command.getExtendedProps<BsCreateUserBranchCommand>();
