...
import { BsCreateUserBranchCommand } from './bs-create-user-branch.command';
import { BsCreateUserBranchRequestDto } from './bs-create-user-branch.request.dto';
import { CreateUserBranchServiceResult } from './bs-create-user-branch.service';
...
export class CreateUserBranchHttpController {
  ...
  @Post(routesV1.businessLogic.userBranch.root)
  async create(
    @ReqUser() user: RequestUser,
    @Body() body: BsCreateUserBranchRequestDto,
  ): Promise<UserBranchResponseDto> {
    const command = new BsCreateUserBranchCommand({
      ...body,
